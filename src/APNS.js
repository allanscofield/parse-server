...
function APNS(args) {
  ...
  for (let apnsArgs of apnsArgsList) {
    ...
    conn.on('connected', () => {
      console.log('APNS %n Connection Connected', connIndex);
    });
    ...
    conn.on('timeout', () => {
      console.log('APNS %n Connection Timeout', connIndex);
    });

    conn.on('disconnected', () => {
      console.log('APNS %n Connection Disconnected', connIndex);
    });

    conn.on('socketError', () => {
      console.log('APNS %n Connection Socket Error', connIndex);
    });

    conn.on('transmitted', function(notification, device) {
      console.log('APNS %n Connection Notification transmitted to: %s', connIndex, device.token.toString('hex'));
    });

    this.conns.push(conn);
    connIndex += 1;
  }
  // Sort the conn based on priority ascending
...
function handleTransmissionError(conns, errCode, notification, apnDevice) {
  ...
  if (newConnIndex < 0 || newConnIndex >= conns.length) {
    console.log('APNS can not find vaild cert for %j', apnDevice.token);
    return;
  }
